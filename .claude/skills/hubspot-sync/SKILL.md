---
name: hubspot-sync
description: W1 — sync every open HubSpot deal from the full conversation (Granola, Slack, Lemlist, email) into HubSpot as dated notes, and move deal stages when the evidence warrants. HubSpot is the single source of truth. Use to keep the pipeline current each morning.
---
...
conversation across **Granola, Slack, Lemlist, and email** by matching the deal's
contact emails / company domain. Write one consolidated `[hubspot-ingest]` note to
