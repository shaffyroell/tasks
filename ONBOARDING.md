...
HubSpot (text) · Handled By (AI/AI + Review/Human) · AI Can Do (text) ·
Needs from You (text)
```
...
- `sources` — toggle which of email / slack / granola / HubSpot apply
...
- **Granola** — meeting-notes connector (if used)
- **HubSpot** — connector / private-app token → `HUBSPOT_SETUP.md`
...
`HUBSPOT_TOKEN`) — never mix tenants in one file.
...
- (If HubSpot on) a note landed on the matched deal.
